...
##### 이부분 수정 ######
target_time = datetime.strptime("2026-02-05T15:52:44", "%Y-%m-%dT%H:%M:%S") + timedelta(minutes=5)
RULE_PATH = "/Volumes/cep_demo/network/rules/2.json"
TARGET_TABLE = "cep_demo.network.s2_results"
linkdown_table = "cep_demo.network.s2_snmp_linkdown_events"
equip_table = "cep_demo.network.s2_device_link_topology"
traffic_table = "cep_demo.network.s2_snmp_interface_traffic"
forecast_table = "cep_demo.network.s2_traffic_forecast"
#####################

spark = SparkSession.builder.getOrCreate()


# Step 1: Linkdown 이벤트 조회 쿼리 (지정 timestamp 기준)
def get_step1_query():
    return f"""
    WITH clock AS (
      SELECT date_trunc('minute', TIMESTAMP '{target_time}') - INTERVAL 5 MINUTE AS target_minute
    )
    SELECT DISTINCT
...
step1_df = spark.sql(get_step1_query())
...
step2_df = spark.sql(get_step2_query(), args={"rows_json": rows_json})
...
if alarms:
    ...
    alarm_df = spark.createDataFrame(alarms)
