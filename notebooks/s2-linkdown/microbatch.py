...
# 룰 파일 경로
RULE_PATH = "/Volumes/cep_demo/network/rules/2.json"
TARGET_TABLE = "cep_demo.network.s2_results"

spark = SparkSession.builder.getOrCreate()
spark.conf.set("spark.sql.session.timeZone", "Asia/Seoul")

def getCheckLinkdownDataQuery():
    # Step 1: target_minute = 현재 - 5분 시점의 1분 구간
    return f"""
    SELECT
      *
    FROM cep_demo.network.s2_snmp_linkdown_events
    WHERE event_at >= DATE_TRUNC('minute', current_timestamp()) - INTERVAL 5 MINUTE
      AND event_at < DATE_TRUNC('minute', current_timestamp()) - INTERVAL 4 MINUTE
    """
...
def getCheckTrafficDataQuery():
    return f"""
    SELECT
      collected_at,
      router_ip,
      if_name,
      {format_number_sql('rx_bytes')} AS rx_bytes_fmt
    FROM cep_demo.network.s2_snmp_interface_traffic
    WHERE collected_at >= DATE_TRUNC('minute', current_timestamp()) - INTERVAL 3 MINUTE
      AND collected_at < DATE_TRUNC('minute', current_timestamp()) - INTERVAL 1 MINUTE
...
def get_step1_query():
    ...
    SELECT DISTINCT
      ...
      -- [수정] SQL 레벨에서 KST 문자열로 변환하여 반환
      date_format(clock.target_minute, 'yyyy-MM-dd HH:mm:ss') AS target_minute
    FROM clock
    JOIN cep_demo.network.s2_snmp_linkdown_events e
...
def get_step2_query():
    ...
    d_norm AS (
      SELECT
        ...
        END AS if_name_norm
      FROM cep_demo.network.s2_device_link_topology d
      JOIN params p
    ...
    base AS (
      SELECT
        ...
        d.link_type
      FROM cep_demo.network.s2_snmp_interface_traffic t
      JOIN d_norm d
    ...
    fcst_raw AS (
      SELECT
        ...
        f.phase
      FROM cep_demo.network.s2_traffic_forecast f
      JOIN params p
...
def run_batch(df, batch_id):
    ...
    print("Input Linkdown 데이터 확인 중... ( 5분 전 ~ 4분 전 )")
    df_linkdown_result = spark_local.sql(getCheckLinkdownDataQuery())
    df_linkdown_result.show(truncate=False, n=10)
    print("Input Traffic 데이터 확인 중... ( 3분 전 ~ 1분 전 )")
    df_traffic_result = spark_local.sql(getCheckTrafficDataQuery())
    df_traffic_result.show(truncate=False, n=10)
    # 룰 변경 감지 함수
...
stream_query = (
    ...
    .option("checkpointLocation", "/Volumes/cep_demo/network/checkpoints/s2_results_driver")
    .start()
)
