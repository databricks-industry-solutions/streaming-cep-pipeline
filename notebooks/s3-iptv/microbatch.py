...
# MAGIC %sh
# MAGIC cat /Volumes/cep_demo/network/rules/3-2.json
...
# 룰 파일 경로
RULE_PATH_1 = "/Volumes/cep_demo/network/rules/3-1.json"
RULE_PATH_2 = "/Volumes/cep_demo/network/rules/3-2.json"
TARGET_TABLE = "cep_demo.network.s3_results"

spark = SparkSession.builder.getOrCreate()
spark.conf.set("spark.sql.session.timeZone", "Asia/Seoul")

def getCheckDataQuery():
    # Step 1: 현재 - 4분 ~ 현재 (interval=3 기준)
    # - 알람 발생: 4분 전 ~ 3분 전
    # - 알람 해제 체크 (NOT EXISTS): 3분 전 ~ 현재
    return f"""
    SELECT
      *
    FROM cep_demo.network.s3_olt_alarm_events
    WHERE eventhub_received_at >= DATE_TRUNC('minute', current_timestamp()) - INTERVAL 4 MINUTE
      AND eventhub_received_at < DATE_TRUNC('minute', current_timestamp())
    """

def format_number_sql(col_name):
    """숫자를 K, M, B 단위로 포맷팅하는 SQL CASE 문 반환"""
    return f"""
    CASE
      WHEN {col_name} >= 1e12 THEN CONCAT(ROUND({col_name}/1e12, 1), 'T')
      WHEN {col_name} >= 1e9 THEN CONCAT(ROUND({col_name}/1e9, 1), 'B')
      WHEN {col_name} >= 1e6 THEN CONCAT(ROUND({col_name}/1e6, 1), 'M')
      WHEN {col_name} >= 1e3 THEN CONCAT(ROUND({col_name}/1e3, 1), 'K')
      ELSE CAST({col_name} AS STRING)
    END"""

def getCheckTrafficAvg3nmQuery():
    # Step 2 avg_3nm: 현재 - 12분 ~ 현재 - 3분 (interval=3 기준, 9분 범위)
    return f"""
    SELECT
      collected_at,
      router_ip,
      if_name,
      {format_number_sql('tx_multicast_pkts')} AS mcast_pkt
    FROM cep_demo.network.s3_snmp_interface_traffic
    WHERE collected_at >= DATE_TRUNC('minute', current_timestamp()) - INTERVAL 12 MINUTE
      AND collected_at < DATE_TRUNC('minute', current_timestamp()) - INTERVAL 3 MINUTE
    ORDER BY collected_at DESC
    """

def getCheckTrafficLast1mQuery():
    # Step 2 last_1m: 현재 - 3분 ~ 현재 - 2분 (interval=3 기준, 1분 범위)
    return f"""
    SELECT
      collected_at,
      router_ip,
      ...
      {format_number_sql('tx_multicast_pkts')} AS mcast_pkt
    FROM cep_demo.network.s3_snmp_interface_traffic
    WHERE collected_at >= DATE_TRUNC('minute', current_timestamp()) - INTERVAL 3 MINUTE
...
def run_batch(df, batch_id):
    ...
    # Input 데이터 확인
    print("Input OLT 알람 데이터 확인 중... ( 4분 전 ~ 현재 )")
    df_olt_result = spark_local.sql(getCheckDataQuery())
    df_olt_result.show(truncate=False, n=10)

    print("Input Traffic 데이터 (avg_3nm) 확인 중... ( 12분 전 ~ 3분 전 )")
    df_traffic_avg = spark_local.sql(getCheckTrafficAvg3nmQuery())
    df_traffic_avg.show(truncate=False, n=20)

    print("Input Traffic 데이터 (last_1m) 확인 중... ( 3분 전 )")
    df_traffic_last = spark_local.sql(getCheckTrafficLast1mQuery())
    df_traffic_last.show(truncate=False, n=10)
...
stream_query = (
    ...
    .option("checkpointLocation", "/Volumes/cep_demo/network/checkpoints/s3_results_driver")
    .start()
)
